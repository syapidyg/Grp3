...
int distVC=SharpIR.getDistance();// gets the distance in cm
  vitesse() ;
  distanceR(distVC ,vlineaire );
transmitter();
movement();
cesar_crypted(decallage, message);      // fonction de decallage
 ...
 int distanceR(int variable , float variable1){
  ...
  if(variable >= 5){
   ...
   String intervale = String(interval);
    Serial.println("Distance:" +intervale+"cm");
  }else{
...
strcat(message,dis);           //concatenation  du message et du code de la distance
